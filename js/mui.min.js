...
(function($){function getType(elem){return(elem.length?elem[0]:elem).type;}
function isExist(value){return'undefined'!==typeof value&&null!==value;}
function formatObject(name,value,result,type){var nameList=name.split('.'),key,k,setValue=function(k,v){if(!isExist(result[k])){result[k]=v||{};}
result=result[k];};if('string'===typeof value){value=$.trim(value);if(''===value){return;}
if('number'===type){value=parseFloat(value);}}
while((key=nameList.shift())){if(~key.indexOf('[')){key=key.split('[');setValue(key.shift(),[]);while((k=key.shift())){k=k.slice(0,-1);if(key.length){setValue(k,[]);}else{if(nameList.length){setValue(k);}else{result[k]='checkbox'===type?[value]:value;}}}}else{if(nameList.length){setValue(key);}else{if(isExist(result[key])){if($.isArray(result[key])){result[key].push(value);}else{result[key]=[result[key],value];}}else{result[key]='checkbox'===type?[value]:value;}}}}}
$.fn.serializeObject=function(callback){"use strict";var result={},_this=this;$.each(_this.serializeArray(),function(i,elem){formatObject(elem.name,elem.value,result,getType(_this[0].elements[elem.name]));});callback&&callback(result);return result;};})(Zepto);
Date.prototype.format=function(formation){var year=this.getFullYear(),month=this.getMonth()+1,date=this.getDate(),hour=this.getHours(),lhour=hour>12?hour-12:hour,minute=this.getMinutes(),second=this.getSeconds(),toTwoDigit=function(num){return num<10?"0"+num:num;},formatObj={yyyy:year,yy:year.toString().slice(2),MM:toTwoDigit(month),M:month,dd:toTwoDigit(date),d:date,HH:toTwoDigit(hour),H:hour,hh:toTwoDigit(lhour),h:lhour,mm:toTwoDigit(minute),m:minute,ss:toTwoDigit(second),s:second};return formation.replace(/y+|m+|d+|h+|s+|H+|M+/g,function($0){return formatObj[$0]||$0;});};
mui.fn.toArray=function(){return [].slice.call(this);}
mui.HEIGHT_HEADER='48px';
mui.HEIGHT_FOOTER='50px';
mui.HEIGHT_ZERO='0px';
...
var T = {
	...
	KEY_USER: 'user',
	KEY_SID: 'sid',
	setItemJSON: function(key, json) {
		plus.storage.setItem(key, JSON.stringify(json));
	},
	getItemJSON: function(key) {
		return mui.parseJSON(plus.storage.getItem(key));
	},
	
	setSid: function(sid) {
		this._SID = sid;
		plus.storage.setItem(this.KEY_SID, sid);
	},
	
	getSid: function(sid) {
		return this._SID || plus.storage.getItem(this.KEY_SID);
	},
	
	DOMAIN: 'http://lifa.chengsekongjian.com',
	
	log: function() {
		console.info([].slice.apply(arguments).map(function(item) {
			return typeof item === 'object' ? JSON.stringify(item) : item;
		}).join('\n'));
	},
	
	error: function() {
		console.error([].slice.apply(arguments).map(function(item) {
			return typeof item === 'object' ? JSON.stringify(item) : item;
		}).join('\n'));
	},
	...
	/**
     * 统一的ajax数据加载
     * @example `T.getDate(url, { success: function(data) {}, unsuccess: function(o) {}, error: function(){}});`
     * @param {String} url
     * @param {Object} config
    ...
    getData: function(url, config) {
        ...
        mui.extend(defaultConfig, config);
        
        defaultConfig.success = function(o) {
        	T.log(o);
            // o = o || {};
            if ('object' !== typeof o) {
	            try {
            ...
            if (o.status === 0) {
                config.success(o.data || '');
            } else if (o.status === 999) { // not login
                mui.alert('您还没有登录，请登录后完成操作...');
                return T.Events.emit('login');
            } else {
                config.unsuccess ? config.unsuccess(o) : o.msg && mui.alert(o.msg);
            }
        };
        ...
        if (sid) {
        	url = url + (~url.indexOf('?') ? '&' : '?') + 'sid=' + sid;
        }
        
        T.log(url, config.data);
        return mui.ajax(url, defaultConfig);
    },
    
    _ajax: function(args, type) {
    	var Events = this.inheritEvents();
    	// [url, data, successFn, unsuccessFn]
    	...
    	this.getData(args[0], {
    		...
    		complete: function() {
    			Events.emit('complete', arguments);
    		}
    	});
    	return Events;
    },
    ...
    get: function() {
		return this._ajax(arguments, 'GET');
    },
    
    post: function() {
		return this._ajax(arguments, 'POST');
    },
    
    put: function() {
    	return this._ajax(arguments, 'PUT');
    },
    
    "delete": function() {
		return this._ajax(arguments, 'DELETE');
    }
};
(function(T) {
	function Events() {
	...
	mui.extend(Events.prototype, {
        ...
        on: function(name, callback) {
            (this._e[name] || (this._e[name] = [])).push(callback);
            return this;
        },
	...
	T.inheritEvents = function(fn) {
		return fn ? T.inherit(fn, Events) : Events();
	};
})(T);
